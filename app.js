...
app.use(function(err, req, res, next) {
    ...
    var messageTrans = {
        ...
        'Data incomplete': '学校教务系统返回的数据不完整，我们无法解析。出现这个问题可能是由于我们或学校的服务器网络情况不太好',
        'UID not exist'  : '你的会话已过期，当你访问首页时会开始一个新的会话，这个会话保质期为15分钟。所以请不要尝试将查询结果页面保存到书签中（应该保存主页）',
        'Unexpected page': '学校教务系统返回了一些我们预料之外的数据，我们不知道接下来该做什么，所以决定将你带回主页。出现这个消息可能是会话已过期需要重新登录',
        'Login failed'   : '你没有成功登录，请检查学号、密码以及验证码是否输入正确，注意密码为教务系统密码'
    }
