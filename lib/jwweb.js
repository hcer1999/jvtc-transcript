...
module.exports = class {
    ...
    async init() {
        ...
        await request(LOGIN_PAGE_URL, {timeout: 12000}).then(({response, body}) => {
            ...
            } else throw new Error('Data Incomplete');
        });
    ...
    async getResults(semester) {
        ...
        if(this.resultCache[semester]) {
            return {
                id: this.userid,
                name: this.username,
                transcript: this.resultCache[semester]
            };
        }
        ...
        if(html.search('成绩认定记录') !== -1) {
            ...
            this.resultCache[semester] = transcript;
            return {
                id: this.userid,
                name: this.username,
                transcript
