...
class JwwebUser {
  ...
  async isLogin() {
    const { res, body } = await request('http://jiaowu.jvtc.jx.cn/jsxsd/framework/xsMain.jsp', {
...
function getHandledTranscript(html) {
  ...
  while ((courseHTML = courseRegexp.exec(html)) != null) {
    ...
    while ((columnHTML = columnRegexp.exec(courseHTML)) != null) {
      matchArr.push(columnHTML[1].trim());
    }
    if (matchArr.length > 12) {
      let course = {};
      // 学期
      ...
      // 成绩
      course['score'] = matchArr[5];
      // 学分
      course['credit'] = matchArr[7];
      // 形式
      course['type'] = matchArr[14];
