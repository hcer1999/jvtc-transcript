...
function sendRequest(userOptions) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        let rq = request(options, function(err) {
            if(err) return reject(err);
        });

        let response;
        let wholeData = new Uint8Array(0);
        rq.on('response', function(res) {
            response = res;
        })
        .on('data', function(data) {
            let tmp = new Uint8Array(wholeData.length + data.length);
            tmp.set(wholeData);
            tmp.set(data, wholeData.length);
            wholeData = tmp;
        })
        .on('end', function() {
            return resolve({response, body: wholeData});
        });
