...
const URL_LOGIN      = 'http://218.65.5.214:2001/jwweb/_data/index_LOGIN.aspx';
const URL_VIEW_SCORE = 'http://218.65.5.214:2001/jwweb/xscj/c_ydcjrdjl.aspx';
const URL_POST_SCORE = 'http://218.65.5.214:2001/jwweb/xscj/c_ydcjrdjl_rpt.aspx';
const URL_CAPTCHA    = 'http://218.65.5.214:2001/jwweb/sys/ValidateCode.aspx';
const URL_LOGOUT     = 'http://218.65.5.214:2001/jwweb/sys/Logout.aspx';
const URL_FOOTER     = 'http://218.65.5.214:2001/jwweb/PUB/foot.aspx';
...
module.exports = class {
    ...
    async init() {
        ...
        let {response, body} = await request(URL_LOGIN, {timeout: 12000});
        let sessionVal;
        try {
    ...
    async getCaptcha() {
        let {body: imageData} = await request(URL_CAPTCHA, {
            headers: {
    ...
    async isLogin() {
        let {body} = await request(URL_VIEW_SCORE, { 
            headers: {
                Cookie: this.sessionCookie
            }
        });
    ...
    async getStudentName() {
        let {body} = await request(URL_FOOTER, {
            headers: {
                Cookie: this.sessionCookie
            }
        });
    ...
    async getResults(semester) {
        ...
        let {body} = await request(URL_POST_SCORE, { 
            method: 'POST', 
            form: {
                sel_xnxq: semester, 
                radCx: '1'
    ...
    async logout() {
        this._isLogin && await request(URL_LOGOUT, {
            headers: {
                Cookie: this.sessionCookie
            }
        });
